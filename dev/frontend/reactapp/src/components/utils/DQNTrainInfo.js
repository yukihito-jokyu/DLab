...
import './DQNTrainInfo.css'

function DQNTrainInfo() {
  ...
  const [syns, setsyns] = useState('20');
  ...
  const handleSynsChange = (e) => {
    setsyns(e.target.value);
  };
  ...
  return (
    <div>
      ...
      <div id='TrainInfo-wrapper'>
        ...
        <div className='Syns'>
          <label>同期のタイミング：</label>
          <input type='number' min='0' onChange={handleSynsChange} step='1' />
          <p className='syns-num'>{syns}</p>
        </div>
      </div>
    </div>
  )
