...
def train_model(config):
    ...
    for epoch in range(train_info["epoch"]):
        ...
        print(f'Epoch [{epoch+1}/{train_info["epoch"]}], Train Acc: {epoch_acc:.4f}, Val Acc: {val_acc:.4f}, Train Loss: {epoch_loss:.4f}, Val Loss: {val_loss:.4f}')
    
    # 学習曲線の保存
    user_id = config["user_id"]
    project_name = config["Project_name"]
    model_id = config["model_id"]
    photo_dir = os.path.abspath(os.path.join(os.getcwd(), "../user", user_id, project_name, model_id, "photo"))
